...
try:
    ...
    sites = [
        { "name": "UU Pure " + config['PURE_PARAMS']['PURE_SYSTEM'] + " " + str(num_records),  "url": config['PURE_PARAMS']['PURE_URI'],    "api-key": config['PURE_PARAMS']['PURE_API_KEY'] },
    ]
    ...
    try:
        ...
        while more_records and run_times < int(num_runs):
            ...
            if save:
                ...
                save_file(response, path_output + "/" + endpoint.replace('/', '-'), "{0}_{1}_{2}".format("{:%Y%m%d}".format(datetime.datetime.now()),endpoint.replace('/', '-'), "{0:0>6}".format(start_offset)), fileType)
                num_logLine += 1 
...
def save_file(response, folder, name, fileType):
    try: 
